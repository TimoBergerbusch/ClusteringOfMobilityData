...
\Cref{fig:OrgDist} is the result of the first try. \Cref{fig:OrgSt} shows the strata distribution as pie chart and \Cref{fig:OrgCl} the resulted cluster distribution. It can already been seen, that the distributions are not similar. For more steps the outcome is analogous.
...
For a clustering of 6 clusters we get a distribution, which can be seen in \Cref{fig:PCAOrgDist}, similar to the one we get from the k-means process with the same configurations. 
For a clustering of 3 clusters we again get results, that are unclear(\Cref{fig:PCAOrgCl3}).
