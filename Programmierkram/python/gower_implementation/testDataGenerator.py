...
DATA_NAMES = ["origin", "destination", "reason", "HMV", "RED", "meanOfTransportation", "averageTime", "duration",
...
class DataCollection:
    ...
    def printToFile(self, fileName, mask):
        ...
        with open(fileName, "w+") as file:
            ...
            file.write("\n")
            for entry in self.data_entries:
                file.write(''.join(entry.getData(mask))) #possible typo, empty string
                file.write("\n")
    ...
    def toDict(self, mask):
        data = dict([(key, []) for key in get_used_headers(mask).split()])
        
        for entry in self.data_entries:
          arr = entry.getData(mask).split()
